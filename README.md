...
Optional TypePHP native accelerator for [eloquage/beacon](https://github.com/eloquage/beacon).
...
```sh
pie install eloquage/beacon-ext
```
...
```
php_eloquage_beacon-X.Y.Z_php8.5-x86_64-Linux-glibc-N-debug-NTS.tar.gz
```

The archive contains `eloquage_beacon.so`. Windows, macOS, ZTS, and non-8.5 binaries are not currently published.
